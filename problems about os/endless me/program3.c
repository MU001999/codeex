...
void create_child(int signal)
{
	...
	else if (pid == 0)
	{
		...
		sprintf(ins, "%d", n);
		sigset_t set; sigemptyset(&set); sigprocmask(SIG_SETMASK, &set, NULL); if (execl(name, name, ins, NULL) < -1) exit(0);
	}
	else exit(0);
}
